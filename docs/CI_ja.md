...
Ubuntu の Python 3.10 / 3.13 を指定しています。性能比較器の試験用にのみ、
NumPy 1.26.4 / 2.3.5をそれぞれ導入します。ゲーム本体に追加インストールは不要です。
...
M0-03 では src も構文検査し、unittest の前に `build_single_file.py --check` を実行します。
生成漏れを自動修復して見逃すことなく拒否し、単体生成・配布の新しい回帰も同じ CI で実行します。
3.10のジョブでは開発用JDK17でコンベア、ドリルの定期dump、生産offload、
ドリル・ベルト・分配器の連続搬送の抽出Java比較も実行します。
3.13ではJavaの手順は設定上の対象外です。抽出比較は原作ゲーム全体やiPhoneの実行ではありません。
